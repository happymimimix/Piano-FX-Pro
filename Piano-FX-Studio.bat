...
ECHO.end >>"%~dp0x"
ECHO.end >>"%~dp0x"
ECHO.function CalculateShaderFPS() >>"%~dp0x"
ECHO.if LastUpdate==nil then LastUpdate=GetMicroseconds() end >>"%~dp0x"
ECHO.local FrameTime=GetMicroseconds()-LastUpdate+1 >>"%~dp0x"
ECHO.local FPS=math.ceil((S/FrameTime)*100.0)/100.0 >>"%~dp0x"
ECHO.LastUpdate=GetMicroseconds() >>"%~dp0x"
ECHO.return FPS >>"%~dp0x"
ECHO.end >>"%~dp0x"
ECHO.function UpdateShaderFPSDisplay() >>"%~dp0x"
ECHO.if DisplayShaderFPS then >>"%~dp0x"
ECHO.local Text=^"Shader FPS: ^"..CalculateShaderFPS() >>"%~dp0x"
...
ECHO.local MEMdc_Main=EXE(^"CreateCompatibleDC^",GDIdc) >>"%~dp0x"
ECHO.local BMP_Main=EXE(^"CreateCompatibleBitmap^",GDIdc,W,H) >>"%~dp0x"
ECHO.local MEMdc_BG=EXE(^"CreateCompatibleDC^",GDIdc) >>"%~dp0x"
ECHO.local BMP_BG=EXE(^"CreateCompatibleBitmap^",GDIdc,W,H) >>"%~dp0x"
ECHO.EXE(^"SelectObject^",MEMdc_R,BMP_R) >>"%~dp0x"
ECHO.local Brush=EXE(^"CreateSolidBrush^",0x0000FF) >>"%~dp0x"
ECHO.EXE(^"SelectObject^",MEMdc_R,Brush) >>"%~dp0x"
ECHO.EXE(^"PatBlt^",MEMdc_R,0,0,W,H,PATCOPY) >>"%~dp0x"
...
ECHO.EXE(^"SelectObject^",MEMdc_BG,BMP_BG) >>"%~dp0x"
ECHO.EXE(^"BitBlt^",MEMdc_BG,0,0,W,H,PFXdc,0,0,SRCCOPY) >>"%~dp0x"
ECHO.local BlendFunc=(Transparency^<^<16)^|0x00000000 >>"%~dp0x"
ECHO.EXE(^"AlphaBlend^",MEMdc_BG,0,0,W,H,MEMdc_Main,0,0,W,H,BlendFunc) >>"%~dp0x"
ECHO.local Brush=EXE(^"CreateSolidBrush^",0x000000) >>"%~dp0x"
ECHO.EXE(^"SelectObject^",MEMdc_BG,Brush) >>"%~dp0x"
ECHO.local i=math.random(0,2) >>"%~dp0x"
ECHO.while i^<=H do >>"%~dp0x"
ECHO.EXE(^"PatBlt^",MEMdc_BG,0,i,W,1,PATCOPY) >>"%~dp0x"
ECHO.i=i+3 >>"%~dp0x"
ECHO.end >>"%~dp0x"
ECHO.EXE(^"DeleteObject^",Brush) >>"%~dp0x"
ECHO.EXE(^"BitBlt^",GDIdc,0,0,W,H,MEMdc_BG,0,0,SRCCOPY) >>"%~dp0x"
ECHO.EXE(^"DeleteObject^",BMP_R) >>"%~dp0x"
ECHO.EXE(^"DeleteDC^",MEMdc_R) >>"%~dp0x"
ECHO.EXE(^"DeleteObject^",BMP_G) >>"%~dp0x"
ECHO.EXE(^"DeleteDC^",MEMdc_G) >>"%~dp0x"
...
ECHO.EXE(^"DeleteObject^",BMP_Main) >>"%~dp0x"
ECHO.EXE(^"DeleteDC^",MEMdc_Main) >>"%~dp0x"
ECHO.EXE(^"DeleteObject^",BMP_BG) >>"%~dp0x"
ECHO.EXE(^"DeleteDC^",MEMdc_BG) >>"%~dp0x"
ECHO.end >>"%~dp0x"

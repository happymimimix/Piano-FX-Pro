...
idx_t MIDIPos::GetNextEvent(mms_t iMicroSecs, MIDIEvent** pOutEvent)
{
    ...
    *pOutEvent = NULL;
    ...
    if (iMicroSecs < 0 || pMinEvent->GetAbsTick() <= iMaxTickAllowed)
    {
        // How many micro seconds did we just process?
        *pOutEvent = pMinEvent;
        mtk_t iSpan = pMinEvent->GetAbsTick() - m_iCurrTick;
        if (m_bIsStandard)
        ...
        m_iCurrTick = pMinEvent->GetAbsTick();
        m_iCurrMicroSec = 0;

        // Advance this track's head pointer + key, then re-balance the tree.
        m_pTrackPos[iMinPos]++;
        m_pTrackTime[iMinPos] = m_pTrackPos[iMinPos] == m_MIDI.m_vTracks[iMinPos]->m_vEvents.size() ? INT64_MAX : m_MIDI.m_vTracks[iMinPos]->m_vEvents[m_pTrackPos[iMinPos]]->GetAbsTick();
...
fileln_t MIDI::ParseTracksF3(const unsigned char* pcData, fileln_t iMaxSize)
{
    track_t iTrack = m_vTracks.size();
    g_LoadingProgress.stage = MIDILoadingProgress::Stage::ParseTracks;
    g_LoadingProgress.progress = 0;
...
bool MIDI::PostProcess(vector<MIDIChannelEvent*>& vChannelEvents, vector<MIDIMetaEvent*>* vMetaEvents, eventvec_t* vTempo, eventvec_t* vSignature, eventvec_t* vMarkers, eventvec_t* vColors, vector<MIDISysExEvent*>* vSysExEvents)
{
    ...
    for (track_t i = 0; i < m_vTracks.size(); i++) {
        ...
        if (!m_vTracks[i]->m_vEvents.empty())
            midiPos.m_pTrackTime[i] = m_vTracks[i]->m_vEvents[0]->GetAbsTick();
    }
...
fileln_t MIDITrack::ParseTrackF3(const unsigned char* pcData, fileln_t iMaxSize)
{
    fileln_t iTotal = 0;
    // Reset first
    clear();
    // Read track name
    fileln_t TrackNameLength = 0;
    iTotal += MIDI::Parse32BitLE(pcData + iTotal, iMaxSize - iTotal, reinterpret_cast<uint32_t*>(&TrackNameLength));
    TrackNameLength = min(TrackNameLength, iMaxSize - iTotal);
    char* Buffer = new char[TrackNameLength];
...
fileln_t MIDITrack::ParseEvents(const unsigned char* pcData, fileln_t iMaxSize, track_t iTrack)
{
    ...
    while (iMaxSize - iTotal > 0 && iCount > 0 && (pEvent->GetEventType() != MIDIEvent::MetaEvent || reinterpret_cast<MIDIMetaEvent*>(pEvent)->GetMetaEventType() != MIDIMetaEvent::EndOfTrack));
...
void MIDITrack::MIDITrackInfo::AddEventInfo(const MIDIEvent& mEvent)
{
    ...
    this->iEventCount++;
    this->iTotalTicks = max(this->iTotalTicks, mEvent.GetAbsTick());
    ...
    case MIDIEvent::MetaEvent:
    {
        ...
        if (eMetaEventType == MIDIMetaEvent::SequenceName)
        {
...
fileln_t MIDIEvent::MakeNextEvent(MIDI& midi, const unsigned char* pcData, fileln_t iMaxSize, track_t iTrack, MIDIEvent** pOutEvent)
{
    ...
    (*pOutEvent)->m_iAbsTick = iDT;
    if (pPrevEvent) (*pOutEvent)->m_iAbsTick += pPrevEvent->m_iAbsTick;
...
fileln_t MIDI::Parse64BitLE(const unsigned char* pcData, fileln_t iMaxSize, uint64_t* piOut)
{
    ...
    *piOut = *reinterpret_cast<uint64_t*>(const_cast<unsigned char*>(pcData));
    return 8;
}
...
fileln_t MIDI::Parse32BitLE(const unsigned char* pcData, fileln_t iMaxSize, uint32_t* piOut)
{
    if (!pcData || !piOut || iMaxSize < 4) return 0;
    // No need to shift digits around here, we're already on little endian.
    *piOut = *reinterpret_cast<uint32_t*>(const_cast<unsigned char*>(pcData));
    return 4;
}
...
fileln_t MIDI::Parse16BitLE(const unsigned char* pcData, fileln_t iMaxSize, uint16_t* piOut)
{
    if (!pcData || !piOut || iMaxSize < 2) return 0;
    // No need to shift digits around here, we're already on little endian.
    *piOut = *reinterpret_cast<uint16_t*>(const_cast<unsigned char*>(pcData));
    return 2;
}

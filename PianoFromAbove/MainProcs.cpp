                ...
                case ID_FILE_PRACTICESONGCUSTOM:
                {
                    ...
                    if (GetOpenFileName(&ofn)) {
                        if (cPlayback.GetPlayMode()) { //Close the current file first before opening another one! 
                            ...
                            SetWindowText(g_hWnd, L"Piano-FX Pro | Made by: happy_mimimix | Ver 3.01 | Now playing: None");
...
BOOL PlayFile( const wstring &sFile, bool bCustomSettings )
{
    ...
    TCHAR sTitle[1<<10];
    if (cViz.bDumpFrames) {
        _stprintf_s(sTitle, TEXT("Piano-FX Pro | Made by: happy_mimimix | Ver 3.01 | Now rendering: %ws"), sFile.c_str() + (sFile.find_last_of(L'\\') + 1));
    }
    else {
        _stprintf_s(sTitle, TEXT("Piano-FX Pro | Made by: happy_mimimix | Ver 3.01 | Now playing: %ws"), sFile.c_str() + (sFile.find_last_of(L'\\') + 1));
    }
